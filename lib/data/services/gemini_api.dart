...
class GeminiApiService {
    ...
    try {
      ...
      final requestBody = {
        "contents": [
          {
            "parts": [
              {"text": "Analyze this math problem and explain it step-by-step. Provide the output in JSON format with the following structure: { \"problemId\": \"unique_id\", \"steps\": [\"Step 1 explanation\", \"Step 2 explanation\"], \"hint\": \"A helpful hint\", \"explanation\": \"Final answer and summary\" }. Description: $description"},
              {
                "inline_data": {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['candidates'] != null && 
          ...
          return Explanation(
            id: parsedContent['problemId']?.toString() ?? DateTime.now().millisecondsSinceEpoch.toString(),
  ...
  List<Step> _convertSteps(dynamic steps) {
    ...
    convertedSteps.add(Step(
      ...
      title: 'Your Turn',
      explanation: 'Now that you have seen the steps, try to solve the problem yourself to verify your understanding!',
    ));
